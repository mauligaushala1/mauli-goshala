...
// ──────────────────────────────────────────────
// NEWS MEDIA — just paste a link, sizing is automatic
// Inside a box's .news-card-media, each photo/video is a plain
//   <div class="news-media-tile"><img src="LINK" alt="…" /></div>
// No manual size classes needed:
//   • 1 tile in a box  → fills the whole box (hero style)
//   • 2–6 tiles in a box → automatically arranged into a bento
//     mosaic, biggest tile first
// And LINK can be a normal image URL OR a YouTube link (any
// youtube.com/watch, youtu.be, embed or /shorts/ link) — a
// YouTube link is automatically turned into a real, working
// video player in that same spot. Mixing photos and videos in
// the same box works too, in any order. Any tile-full/
// tile-large/tile-wide/tile-tall class already on a tile is
// ignored and recalculated automatically, so it's safe to
// leave old classes in place or add tiles without any class.
// ──────────────────────────────────────────────
(function initNewsMedia() {
  // How tiles are sized based on how many are in one box.
  // '' means a plain small tile (1 col × 1 row).
  const LAYOUTS = {
    1: ['tile-full'],
    2: ['tile-large', 'tile-large'],
    3: ['tile-large', 'tile-wide', 'tile-wide'],
    4: ['tile-large', 'tile-wide', '', ''],
    5: ['tile-large', '', '', '', ''],
    6: ['tile-wide', 'tile-wide', '', '', '', '']
  };
  ...
  document.querySelectorAll('.news-card-media').forEach(box => {
    const tiles = Array.from(box.querySelectorAll('.news-media-tile'));
    const layout = LAYOUTS[tiles.length] || tiles.map(() => '');

    tiles.forEach((tile, i) => {
      tile.classList.remove('tile-full', 'tile-large', 'tile-wide', 'tile-tall');
      if (layout[i]) tile.classList.add(layout[i]);

      const img = tile.querySelector('img');
      if (!img) return; // already converted (e.g. on repeat init), skip
      const id = youtubeId(img.getAttribute('src'));
      if (id) {
        const iframe = document.createElement('iframe');
        iframe.src = `https://www.youtube.com/embed/${id}`;
        iframe.title = img.getAttribute('alt') || 'YouTube video';
...
(function initNewsTabs() {
  ...
  // Wire up the left/right arrows for every panel's row — works no
  // matter how many tabs or cards get added later. This finds
  // whichever box is closest to centre and scrolls exactly to the
  // next/previous box's real position (rather than guessing by a
  // fixed pixel width, which is what caused the arrows to get
  // "stuck" if a box's actual rendered width ever drifted even
  // slightly from row.clientWidth — e.g. from a scrollbar, a
  // missing image collapsing a box's size, or rounding).
  document.querySelectorAll('.news-row-wrap').forEach(wrap => {
    ...
    function currentIndex() {
      ...
      cards.forEach((card, i) => {
        const dist = Math.abs(card.offsetLeft - row.scrollLeft);
    ...
    function goTo(i) {
      ...
      row.scrollTo({ left: cards[idx].offsetLeft, behavior: 'smooth' });
    }
